...
public sealed class OrdersController(IQuotationService service) : ControllerBase
{
    [HttpPost("/quotations/{quotationId:int}/orders/{orderId:int}"), RequirePermission(QuotationPermissions.OrdersWrite, ResourcePathTemplate = "/quotations/{quotationId}", RequireLiveCheck = true)] public async Task<IActionResult> CreateQuotationOrderLinkAsync(int quotationId, int orderId, CancellationToken ct) { var value = await service.CreateOrderLinkAsync(quotationId, orderId, ct); return value is null ? NotFound() : CreatedAtRoute("GetQuotationOrder", new { id = value.Id }, value); }
