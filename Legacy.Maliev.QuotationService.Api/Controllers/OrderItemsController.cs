...
public sealed class OrderItemsController(IQuotationService service) : ControllerBase
{
    [HttpPost, RequirePermission(QuotationPermissions.LinesWrite, RequireLiveCheck = true)] public async Task<IActionResult> CreateOrderItemAsync(UpsertQuotationOrderItemRequest item, CancellationToken ct) { var value = await service.CreateOrderItemAsync(item, ct); return CreatedAtRoute("GetOrderItem", new { orderItemId = value.Id }, value); }
