...
public sealed class ExpiredQuotationWorker(IServiceScopeFactory scopeFactory, TimeProvider timeProvider, ILogger<ExpiredQuotationWorker> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        do
        {
            try { await using var scope = scopeFactory.CreateAsyncScope(); var changed = await scope.ServiceProvider.GetRequiredService<IQuotationService>().DeclineExpiredAsync(DateTime.SpecifyKind(timeProvider.GetUtcNow().UtcDateTime, DateTimeKind.Unspecified), stoppingToken); if (changed > 0) logger.LogInformation("Declined {QuotationCount} expired quotations", changed); }
