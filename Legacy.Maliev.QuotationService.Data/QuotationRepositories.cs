    ...
    TimeProvider timeProvider) : IQuotationService
{
    ...
    private static QuotationRequestResponse ToResponse(QuotationRequest x) => new(x.Id, x.FirstName, x.LastName, x.Email, x.TelephoneNumber, x.Country, x.CompanyName, x.TaxIdentification, x.Message, x.InternalComment, x.Done, x.CreatedDate, x.ModifiedDate, x.JourneyId, x.TransactionId ?? TransactionId(x.Id));
